...
class Token : public TreeNode {  // The Token class is now derived from TreeNode
 ...
 public:
  ...
  Token(int lineIndex, int posIndex)
      : m_lineIndex(lineIndex), m_posIndex(posIndex) {}

  virtual const std::string& getStringValue() const = 0;
  virtual bool has_string_value(const std::string& aString) {
    return getStringValue() == aString;
  };
  ...
  void print(std::ostream& outs) const {
    outs << demangle(typeid(*this).name()) << " (line: " << m_lineIndex
         << ", pos: " << m_posIndex << ")"
         << " : " << this->getStringValue();
  }
...
class TokenException : public std::exception {
 ...
 public:
  TokenException(TokenSP tkn = nullptr, std::string msg = "") {
    if (tkn)
      ErrMsg = "\nSyntax Error around {" + tkn->getStringValue() + "}, line " +
               std::to_string(tkn->getLineIndex()) + ", index " +
               std::to_string(tkn->getPosIndex()) + "\n" + msg + "\n";
    else
      ErrMsg = "Syntax Error!\n" + msg + "\n";
  }
...
class StringBasedToken : public Token {
 ...
 public:
  StringBasedToken(const std::string& aStringValue, int lineIndex, int posIndex)
      : Token(lineIndex, posIndex), m_stringValue(aStringValue){};
  void print(std::ostream& str) const {
    Token::print(str);
    str << ": " << m_stringValue;
  }  // needed ?
     //	bool has_string_value(const std::string& aString) { return m_stringValue
     //== aString; } // override
...
class punctuator : public StringBasedToken {
  // e.g., ';', '*', '(', ')', ':', etc.
 public:
  punctuator(const std::string& puncValue, int lineIndex, int posIndex)
      : StringBasedToken(puncValue, lineIndex, posIndex){};
...
class type_spec : public StringBasedToken {  // 'char' | 'int' | 'float'
 public:
  ...
  type_spec(const std::string& typeValue, int lineIndex, int posIndex)
      : StringBasedToken(typeValue, lineIndex, posIndex){};
...
class id : public StringBasedToken {  // identifiers -- example:  sum
 public:
  id(const std::string& idValue, int lineIndex, int posIndex)
      : StringBasedToken(idValue, lineIndex, posIndex){};
...
class pointer_id : public id {  // id of type char*
 public:
  pointer_id(const std::string& idValue, int lineIndex, int posIndex)
      : id(idValue, lineIndex, posIndex){};
};

class numeric_id : public id {  // id of type float (and see below)
 public:
  numeric_id(const std::string& idValue, int lineIndex, int posIndex)
      : id(idValue, lineIndex, posIndex){};
  TokenSP process_numeric_primary_exp() override {
    return process_numeric_id();
  }
};

class int_id : public numeric_id {  // id of types char and int
 public:
  int_id(const std::string& idValue, int lineIndex, int posIndex)
      : numeric_id(idValue, lineIndex, posIndex){};
  TokenSP process_int_id() override {
    return process_id();
  }  // a different pattern is being followed here.
...
class gen_assignment_operator : public StringBasedToken {
  // '=' | '/=' | '+=' | '-='
 public:
  gen_assignment_operator(const std::string& opValue,
                          int lineIndex,
                          int posIndex)
      : StringBasedToken(opValue, lineIndex, posIndex){};
...
class int_assignment_operator : public StringBasedToken {
  // '%=' | '<<=' | '>>='
 public:
  int_assignment_operator(const std::string& opValue,
                          int lineIndex,
                          int posIndex)
      : StringBasedToken(opValue, lineIndex, posIndex){};
...
class conditional_operator : public StringBasedToken {
  // ?
 public:
  conditional_operator(const std::string& opValue, int lineIndex, int posIndex)
      : StringBasedToken(opValue, lineIndex, posIndex){};
  TokenSP advance_past_conditional_operator() { return process_token(); }
  void print_traverse() override {
...
class shift_operator : public StringBasedToken {
  // '<<' | '>>'
 public:
  shift_operator(const std::string& opValue, int lineIndex, int posIndex)
      : StringBasedToken(opValue, lineIndex, posIndex){};
  TokenSP advance_past_shift_operator() { return process_token(); }
};

class additive_operator : public StringBasedToken {
  // '+' | '-'
 public:
  additive_operator(const std::string& opValue, int lineIndex, int posIndex)
      : StringBasedToken(opValue, lineIndex, posIndex){};
...
class div_operator : public StringBasedToken {  // '/'
 public:
  div_operator(const std::string& opValue, int lineIndex, int posIndex)
      : StringBasedToken(opValue, lineIndex, posIndex){};
...
class mod_operator : public StringBasedToken {  // '%'
 public:
  mod_operator(const std::string& opValue, int lineIndex, int posIndex)
      : StringBasedToken(opValue, lineIndex, posIndex){};
...
class comp_operator : public StringBasedToken {
  //  '~'
 public:
  comp_operator(const std::string& opValue, int lineIndex, int posIndex)
      : StringBasedToken(opValue, lineIndex, posIndex){};
...
class postfix_operator : public StringBasedToken {  // '++' | '--'
 public:
  postfix_operator(const std::string& opValue, int lineIndex, int posIndex)
      : StringBasedToken(opValue, lineIndex, posIndex){};
  // TODO: ***** Complete this class if needed
...
class string : public StringBasedToken {
  // e.g., "sample string"
 public:
  string(const std::string& aStringValue, int lineIndex, int posIndex)
      : StringBasedToken(aStringValue, lineIndex, posIndex){};

  TokenSP process_primary_exp() override { return process_token(); }
};
...
class incorrect : public StringBasedToken {
 ...
 public:
  incorrect(const std::string& aStringValue, int lineIndex, int posIndex)
      : StringBasedToken(aStringValue, lineIndex, posIndex){};
};

template <typename T>
class numeric_const : public constant {  // e.g., 123 | 'a' | 12.5
 private:
  T m_value;
  std::string m_stringValue;

 public:
  numeric_const(const T& constValue, int lineIndex, int posIndex)
      : constant(lineIndex, posIndex),
        m_value(constValue),
  ...
  TokenSP process_numeric_primary_exp() override {
    return this->process_primary_exp();
  }
};

template <typename T>
class int_const : public numeric_const<T> {  // e.g., 123 | 'a'
 public:
  int_const(const T& constValue, int lineIndex, int posIndex)
      : numeric_const<T>(constValue, lineIndex, posIndex) {}
